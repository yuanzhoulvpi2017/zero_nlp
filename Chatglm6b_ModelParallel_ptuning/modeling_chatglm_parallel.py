...
class PrefixEncoder(torch.nn.Module):
    ...
    def forward(self, prefix: torch.Tensor):
        if self.prefix_projection:
            ...
            cur_device = self.embedding.weight.device
            past_key_values = self.embedding(prefix.to(cur_device))
...
class SelfAttention(torch.nn.Module):
    ...
    def attention_fn(
        ...
        if layer_past is not None:
            past_key, past_value = layer_past[0].to(key_layer.device), layer_past[1].to(key_layer.device)
