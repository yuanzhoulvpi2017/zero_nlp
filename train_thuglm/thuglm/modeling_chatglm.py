...
class SelfAttention(torch.nn.Module):
    ...
    def forward(
        ...
        mixed_raw_layer = self.query_key_value(hidden_states.to(torch.half))
        ...
        output = self.dense.to(torch.half)(context_layer)
...
class GLU(torch.nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        intermediate_parallel = self.dense_h_to_4h.to(torch.half)(hidden_states.to(torch.half))

        intermediate_parallel = self.activation_func(intermediate_parallel)

        output = self.dense_4h_to_h.to(torch.half)(intermediate_parallel)
...
class GLMBlock(torch.nn.Module):
    ...
    def forward(
        ...
        # [seq_len, batch, hidden_size]
        attention_input = self.input_layernorm(hidden_states.to(torch.float32))
...
class ChatGLMForConditionalGeneration(ChatGLMPreTrainedModel):
    ...
    def forward(
        ...
        lm_logits = self.lm_head(hidden_states.to(torch.half)).permute(1, 0, 2).contiguous()
